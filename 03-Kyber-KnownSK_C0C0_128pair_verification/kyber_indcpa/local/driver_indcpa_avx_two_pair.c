...
static __attribute__((noinline)) int monitor(void *in)
{
	...
	sprintf(output_filename, "./out/avx_%01d_%03d_%05d_%02d_%06d.out", arg->guess_z, arg->pair_index, arg->number_thread, arg->sk_index, rept_index);
...
int main(int argc, char *argv[])
{
	...
	while ((read = getline(&line, &len, selectors_file)) != -1) {
		...
		// Read selector
		sscanf(line, "%d %d %d %d", &(guess_z[num_selectors]), &(pair_index[num_selectors]), &(number_thread[num_selectors]),&(sk_index[num_selectors]));
	...
	for (int i = 0; i < outer * num_selectors; i++) {
		...
		arg.sk_index = sk_index[i% num_selectors];
		...
		char command[256];
		sprintf(command, "../kyber/avx2/test_kyber_indcpa_local_two_pair  %d %d %d 10000000", arg.guess_z, arg.pair_index, arg.number_thread);
		printf("Running: %s\n", command);
		...
		// Stop stress
		system("pkill -f test_kyber_indcpa_local_two_pair"); 
