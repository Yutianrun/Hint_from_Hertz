...
static __attribute__((noinline)) int monitor(void *in)
{
	...
	sprintf(output_filename, "./out/all_%01d_%03d_%05d_%06d.out", arg->guess_z, arg->pair_index, arg->number_thread, rept_index);
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < outer * num_selectors; i++) {
		...
		char command[256];
		sprintf(command, "../kyber/ref/test_kyber_indcpa_local  %d %d %d 1000000", arg.guess_z, arg.pair_index, arg.number_thread);
		printf("Running: %s\n", command);
