...
void kyber_malice_enc(uint8_t c[KYBER_UNCOMPRESSED_BYTES],
                      const int16_t guess_z, const int pair_index)
  {
    polyvec b;
    poly v;
    ...
    // rand=rand%3329;
    // b.vec[0].coeffs[2*pair_index] = rand;
    // b.vec[0].coeffs[2*pair_index+1] = (rand*guess_z)%3329;
    // }while(rand==0);

    b.vec[0].coeffs[2*pair_index] = 1;
    b.vec[0].coeffs[2*pair_index+1] = guess_z;
  //   printf(" b ");
  // for(int i=0;i<256;i++)
  //     printf("%d ",b.vec[0].coeffs[i]);
  // printf("\n");
...
void kyber_malice_enc_two_pair(uint8_t c[KYBER_UNCOMPRESSED_BYTES],
                      const int16_t guess_z1, const int16_t guess_z2, const int pair_index)
  {
    ...
    b.vec[0].coeffs[0] = 1;
    b.vec[0].coeffs[1] = guess_z1;

    b.vec[0].coeffs[2*pair_index] = 1;
    b.vec[0].coeffs[2*pair_index+1] = guess_z2;

    // b.vec[0].coeffs[2*pair_index] = 1;
    // b.vec[0].coeffs[2*pair_index+1] = 796;
