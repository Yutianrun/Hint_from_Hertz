...
void * kyber_thread(void *args)
{
    ...
    // uint8_t ct_temp[KYBER_UNCOMPRESSED_BYTES];
    // uint8_t key[CRYPTO_BYTES];

    // memcpy(&k_temp, &actual_args->k, SHAREDKEYBYTES);
    // memcpy(&c_temp, &actual_args->c, CIPHERTEXTBYTES);
    // memcpy(&sk_temp, &actual_args->sk, SECRETKEYBYTES);
