...
def parse_file(fn):
    energy = []
    freq = []
    with open(fn,encoding='latin-1') as f:  # latin-1 lets every single-byte value decode without tripping on multi-byte correctness
        for line in f:
            try:
...
def main():
    ...
    # Keep for quick debugging if we want to sort by frequency_mean in descending order
    ...
    #                                     color='DarkSlateGrey'))))
    fig.add_trace(go.Scatter(x=df['z'], y=df['frequency_mean'],
                    mode='markers', name='random hamming weight', marker=dict(color='blue', size=15,  line=dict(width=2,
                                        color='DarkSlateGrey'))))
    ...
    fig.add_trace(go.Scatter(x=[110,597],y=[float(df[df['z']==110]['frequency_mean']),float(df[df['z']==597]['frequency_mean'])],
                    mode='markers', name='low hamming weight', 
                    marker=dict(color='red', size=20, line=dict(width=2,color='DarkSlateGrey',))))      
    ...
    # The following avoids the "Loading [MathJax]/extensions/MathMenu.js" warning when exporting

    # pio.kaleido.scope.mathjax = None    
    pio.full_figure_for_development(fig, warn=False)
    fig.write_image("plot/avx_mean_{}_ToyKyber.pdf".format(len(df)),engine="kaleido")
    time.sleep(2)
    fig.write_image("plot/avx_mean_{}_ToyKyber.pdf".format(len(df)),engine="kaleido")
