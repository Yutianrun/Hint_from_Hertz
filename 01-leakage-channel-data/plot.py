...
def detect_drop(trace):
    best_x = 0
    max_diff = 0
    for x in range(int(len(trace) * 0.25), int(len(trace) * 0.85)):  # FIXME: may need to restrict range
...
def main():
    ...
    for f in out_files:
        ...
        if (plot_raw_freq):
            ...
            plt.ylim(3.8,3.9)
            plt.ylabel('运行频率 (GHz)')
            plt.savefig("./plot/freq_%s_%s.pdf" % (label, rept_idx), bbox_inches='tight')
            plt.clf()
            plt.close()
    ...
    if plot_steady:
        ...
        bins = np.arange(minimum+0.1, maximum+0.3, 0.1)    # FIXME: adjust range
        num, bins, bars = ax.hist(datas, alpha=0.5, bins=bins, weights=weights, label=labels, align="left", density=True)
        for b in bars:
            ax.bar_label(b,labels = ["{:.2f}%".format(i*10) for i in b.datavalues],label_type='edge',fontsize='xx-small')


        # bins = np.arange(minimum+0.1, maximum, 0.1)    # FIXME: adjust range
