...
int main(int argc, char *argv[])
{
	...
	int num_selectors = 0;
	int16_t guess_z1[3329];
	int16_t guess_z2[3329];
	// int keyindex[100];
	int pair_index[3329];
	...
	while ((read = getline(&line, &len, selectors_file)) != -1) {
		...
		sscanf(line,
		       "%hd %hd %d %d",
		       &(guess_z1[num_selectors]),
		       &(guess_z2[num_selectors]),
		       &(pair_index[num_selectors]),
	...
	for (int i = 0; i < outer * num_selectors; i++) {
		...
		arg.guess_z1 = guess_z1[i % num_selectors];
		arg.guess_z2 = guess_z2[i % num_selectors];
		...
		char command[256];
		sprintf(command, "./bin/test_kyber_indcpa_local %d %d %d %d 10000000", arg.guess_z1, arg.guess_z2, arg.pair_index, arg.number_thread);
		// printf("Running: %s\n", command);
