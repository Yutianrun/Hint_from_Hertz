...
SPECIAL_Z1 = 2246
SPECIAL_Z2 = 3165
SPECIAL_Z3 = 1061  # half_zero_z for pair_index=5 when z_fixed=3165
...
def plot_frequency_scatter(df):
    ...
    special_1 = df[df['z']==SPECIAL_Z1]
    special_2 = df[df['z']==SPECIAL_Z2]
    special_3 = df[df['z']==SPECIAL_Z3]

    if not special_1.empty:
        ax.scatter(special_1['z'], special_1['frequency_mean'],
                  color='red', s=500,
                  edgecolors='DarkSlateGrey', linewidths=2, zorder=3,
                  label=r'$(C_2,0,0,...) /(0,C_3,0,...)$')

    if not special_2.empty:
        ax.scatter(special_2['z'], special_2['frequency_mean'],
                  color='red', s=500,
                  edgecolors='DarkSlateGrey', linewidths=2, zorder=3)

    if not special_3.empty:
        ax.scatter(special_3['z'], special_3['frequency_mean'],
                  color='red', s=500,
                  edgecolors='DarkSlateGrey', linewidths=2, zorder=3)
...
def main():
    ...
    for _, row in df.iterrows():
        if row['z'] in [SPECIAL_Z1, SPECIAL_Z2, SPECIAL_Z3]:
