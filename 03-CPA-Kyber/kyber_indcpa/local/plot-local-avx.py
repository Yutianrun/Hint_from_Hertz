...
def main():
    ...
    if plot_freq:
        ...
        fig.add_trace(go.Scatter(x=df['z'], y=df['frequency_mean'],
                        mode='markers', name=r'$\huge (C_0,C_1,0,...)$', marker=dict(color='blue', size=20, opacity=0.7, line=dict(width=2,
                                            color='DarkSlateGrey'))))
        ...
        # fig.add_trace(go.Scatter(x=[1760],y=[float(df[df['z']==1760]['frequency_mean'])],
        ...
        # pio.full_figure_for_development(fig, warn=False)
        fig.write_image("plot/avx_mean_{}.pdf".format(len(df)),engine="kaleido")
        print("low hamming weight case average frequency :",np.mean([df[df['z']==1760]['frequency_mean'],df[df['z']==2923]['frequency_mean']]))
        print("nomral hamming weight case average frequency :",np.mean(df.loc[(df.z !=1760) & (df.z!=2923),['frequency_mean']]['frequency_mean']))
        # fig = go.Figure()
