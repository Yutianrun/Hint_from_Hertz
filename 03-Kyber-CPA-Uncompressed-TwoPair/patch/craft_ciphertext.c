 ...
 * If z_fixed == 0, pair0 is not set (remains zero).
 */
void craft_ciphertext_two_pair(uint8_t c[KYBER_UNCOMPRESSED_BYTES],
    ...
    /* Fixed known hint pair mapped to pair0 (skip if z_fixed == 0). */
    if (z_fixed != 0) {
        b.vec[0].coeffs[0] = 1;
        b.vec[0].coeffs[1] = z_fixed;
    }

    /* Target pair to attack. */
    b.vec[0].coeffs[2 * target_pair_index] = 1;
    b.vec[0].coeffs[2 * target_pair_index + 1] = z_target;
