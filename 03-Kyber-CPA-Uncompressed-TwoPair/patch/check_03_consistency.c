...
int main(int argc, char **argv) {
  ...
  } else if (argc >= 2 && strcmp(argv[1], "--scan-two-pair") == 0) {
    if (argc != 4) {
      fprintf(stderr, "Usage: %s --scan-two-pair <z_fixed> <pair_index>\n", argv[0]);
      return 1;
    }
    two_pair_mode = 1;
    z_fixed = (int16_t)atoi(argv[2]);
    pair_index = atoi(argv[3]);
  } else if (argc == 4) {
    z_fixed = (int16_t)atoi(argv[1]);
    z_target = (int16_t)atoi(argv[2]);
    pair_index = atoi(argv[3]);
  } else if (argc != 1) {
    fprintf(stderr, "Usage: %s [z_fixed z_target pair_index]\n", argv[0]);
    fprintf(stderr, "       %s --scan [pair_index]\n", argv[0]);
    fprintf(stderr, "       %s --scan-two-pair <z_fixed> <pair_index>\n", argv[0]);
    return 1;
  }
  ...
  if (two_pair_mode) {
    ...
    for (int z = 0; z < KYBER_Q; z++) {
      uint8_t ct_patch[KYBER_UNCOMPRESSED_BYTES];
      craft_ciphertext_two_pair(ct_patch, z_fixed, (int16_t)z, pair_index);
    ...
    fprintf(stderr, "scan_two_pair_done z_fixed=%d pair_index=%d hits=%d\n",
            z_fixed, pair_index, hits);
    return hits == 2 ? 0 : 2;
  }

  uint8_t ct_patch[KYBER_UNCOMPRESSED_BYTES];
  craft_ciphertext_two_pair(ct_patch, z_fixed, z_target, pair_index);
