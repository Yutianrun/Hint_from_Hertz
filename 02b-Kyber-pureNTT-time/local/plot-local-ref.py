...
def main():
    ...
    fig.add_trace(go.Scatter(x=df['z'], y=df['frequency_mean'],
                    mode='markers', name='CC', marker=dict(color='blue', size=20,  line=dict(width=2,
                                        color='DarkSlateGrey'))))

    fig.add_trace(go.Scatter(x=[1044],y=[float(df[df['z']==1044]['frequency_mean'])],
                    mode='markers', name='C0/0C hit', 
                    marker=dict(color='red', size=20, line=dict(width=2,color='DarkSlateGrey',))))
    fig.update_traces(textposition="bottom right")
    fig.write_image("plot/mean.pdf")
