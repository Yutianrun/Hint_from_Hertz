...
def parse_file(fn):
    energy = []
    freq = []
    with open(fn,encoding='latin-1') as f:  # latin-1 makes every single-byte value decodable so malformed lines do not crash parsing
        for line in f:
            try:
...
def main():
    ...
    # Keep for quick debugging if we need to sort by frequency_mean in descending order
    ...
    fig.add_trace(go.Scatter(x=df['z'], y=df['frequency_mean'],
                    mode='markers', name=r'$\Large (C_0,C_1,C_2,0,...)$', marker=dict(color='blue', size=15,  line=dict(width=2,
                                        color='DarkSlateGrey'))))
    # fig.add_trace(go.Scatter(x=df['z'], y=df['frequency_mean'],
    ...
    fig.add_trace(go.Scatter(x=[942,6684,7359],y=[float(df[df['z']==942]['frequency_mean']),float(df[df['z']==6684]['frequency_mean']),float(df[df['z']==7359]['frequency_mean'])],
                    mode='markers', name=r'${\Large (C_3,C_4,0,0,...) \\ (C_5,0,C_6,0,...) \\ (0,C_7,C_8,0,...)\\~~~\ \quad \_ ~}$', 
                    marker=dict(color='red', size=20, line=dict(width=2,color='DarkSlateGrey',))))    
