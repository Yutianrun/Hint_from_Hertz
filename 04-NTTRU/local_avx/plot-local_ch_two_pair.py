...
def parse_file(fn):
    energy = []
    freq = []
    with open(fn,encoding='latin-1') as f:  # latin-1 keeps parsing tolerant of stray single-byte values
        for line in f:
            try:
...
def main():
    ...
    # Keep for quick debugging if we ever sort by frequency_mean
    ...
    fig.add_trace(go.Scatter(x=df['z'], y=df['frequency_mean'],
                    mode='markers', name=r'$\Large (C_0,C_1,0,...C_{3j},C_{3j+1},C_{3j+2},...,)$', marker=dict(color='blue', size=15,  line=dict(width=2,
                                        color='DarkSlateGrey'))))
    # fig.add_trace(go.Scatter(x=df['z'], y=df['frequency_mean'],
    ...
    fig.add_trace(go.Scatter(x=[4790],y=[float(df[df['z']==4790]['frequency_mean'])],
                    mode='markers', name=r'${\Large (C_0,C_1,0,...,C_{3j},C_{3j+1},0,...,)}$', 
                    marker=dict(color='red', size=20, line=dict(width=2,color='DarkSlateGrey',))))    
    ...
    fig.write_image("plot/avx_mean_{}_NTTRU_two_pair.pdf".format(len(df)),engine="kaleido")
    time.sleep(2)
    fig.write_image("plot/avx_mean_{}_NTTRU_two_pair.pdf".format(len(df)),engine="kaleido")
