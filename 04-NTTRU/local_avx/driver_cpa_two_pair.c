...
static __attribute__((noinline)) int monitor(void *in)
{
	...
	sprintf(output_filename, "./out/all_%01d_%03d_%05d_%06d.out", arg->guess_z, arg->tripple_index, arg->number_thread, rept_index);
	// sprintf(output_filename, "./out/all_%01d_%02d_%03d_%05d_%06d.out", arg->hit_difference, arg->keyindex, arg->bitindex, arg->number_thread, rept_index);
	rept_index += 1;
...
int main(int argc, char *argv[])
{
	...
	int guess_z[7682];
	// int keyindex[100];
	int tripple_index[7682];
	int number_thread[7682];
	size_t len = 0;
	ssize_t read = 0;
	char *line = NULL;
	while ((read = getline(&line, &len, selectors_file)) != -1) {
		...
		// Read selector
		sscanf(line, "%d %d %d", &(guess_z[num_selectors]), &(tripple_index[num_selectors]), &(number_thread[num_selectors]));
	...
	// 	arg.tripple_index = tripple_index[i % num_selectors];
	...
	// 	char command[256];
	// 	sprintf(command, "../NTTRU/avx2/test/test_kem_local  %d %d %d 1", arg.guess_z, arg.tripple_index, arg.number_thread);
	// 	// printf("Running: %s\n", command);
	...
	for (int j = 0; j < outer; j++){
		...
		for (int i = start; i != end + step; i += step)
		{
			...
			arg.guess_z = guess_z[i];
			arg.tripple_index = tripple_index[i];
			arg.number_thread = number_thread[i];
			...
			// Prepare background stress command
			char command[256];
			sprintf(command, "../NTTRU/avx2/test/test_ntru_local_two_pair  %d %d %d 1", arg.guess_z, arg.tripple_index, arg.number_thread);
			// printf("Running: %s\n", command);
			...
			// Stop stress
			system("pkill -f test_ntru_local_two_pair ");
