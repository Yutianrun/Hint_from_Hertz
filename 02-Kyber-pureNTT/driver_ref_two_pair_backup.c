...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < outer;i++)
	{
		...
		randombytes((uint8_t*)&non_zero.coeffs[0],sizeof(non_zero.coeffs[0]));
		randombytes((uint8_t*)&non_zero.coeffs[1],sizeof(non_zero.coeffs[1]));		
		...
		// Fixed: Pass 0 as void* instead of int
		clone(&monitor, tstacks + (ntasks + 1) * STACK_SIZE, CLONE_VM | SIGCHLD, (void*)0);
		...
		// Fixed: Cast int16_t* to uint8_t* for randombytes() compatibility
		randombytes((uint8_t*)&one_zero.coeffs[0],sizeof(one_zero.coeffs[0]));
		...
		sleep(10);
		// Start the monitor thread
		// Fixed: Pass 1 as void* instead of int
		clone(&monitor, tstacks + (ntasks + 1) * STACK_SIZE, CLONE_VM | SIGCHLD, (void*)1);
